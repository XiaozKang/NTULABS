...
void inOrderTraversal(BSTNode *root)
{
    ...
    while(1){
        if(temp!=NULL){ //pushing all left nodes into stack
            push(&s, temp);
            temp=temp->left;
            continue;      //directly ignore below cases if temp!=NULL
        }
        else if(temp==NULL && !isEmpty(&s)){
            temp = pop(&s);
            printf("%d ", temp->item);
            temp = temp->right;   //checking on the right nodes
        }
        else break;  //empty stack and temp == NULL
    }
}
