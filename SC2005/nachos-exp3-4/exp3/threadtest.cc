...
int value=0;
//case 0
void Inc(_int which) //which represents the thread number
{
	int a=value;
	a++;
	value=a;
...
void Dec(_int which) //which represents the thread number
{
	int a=value;
	a--;
	value=a;
...
{
	//fill your code
	//do not need to perform context switch
...
void TestValueOne()
{
	...
	//Call Fork to put all four threads into ready list, must assign t1 and t2 in ready list first to perform context switch in Inv_v1
	t1->Fork(Inc_v1, 1, 1);
	t2->Fork(Inc_v1, 2, 1);

	//Dec_v1 threads dont need to join, use yield() later
	t3->Fork(Dec_v1, 3, 0);
	t4->Fork(Dec_v1, 4, 0);
	
	// main thread gives up CPU and waits for last thread of Inc_v1 before continuing the checking below
	currentThread->Yield();
...
//1. Declare any paramters here.
	...
	//fill your code
	Lock *lock = new Lock ("Special Lock");
