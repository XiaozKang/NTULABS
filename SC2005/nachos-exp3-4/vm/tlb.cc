...
int VpnToPhyPage(int vpn)
{
  ...
  //you can refer to PageOutPageIn(int vpn) to see how an entry was created in ipt
	//NumPhysPages is a constant defined in machine.h, NumPhysPages=4
  	for (int i = 0; i < NumPhysPages; i++){
    	// conditions written in briefing slides
		//conditions to check are valid bit,process id and virtual page number to tally
...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  //your code to find an empty entry in TLB or to replace the oldest entry if TLB is full
  //TLBSize is also a constant defined in machine.h, TLBSize=3
  ...
  FIFOPointer = (i + 1) % TLBSize; // applicable when the  TLB is full

  
  // copy dirty data to memoryTable
  // removing the old entry from the TLB
...
void DoPageOut(int phyPage)
{
  ...
  if(memoryTable[phyPage].valid){           // check if pageOut possible, mainly when IPT is full else dun need to PageOut
...
int lruAlgorithm(void)
{
  ...
  for (int i = 0; i < NumPhysPages; i++)
  {
    // just return entry if it's invalid/empty
